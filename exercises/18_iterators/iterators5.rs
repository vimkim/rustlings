...
fn count_iterator(map: &HashMap<String, Progress>, value: Progress) -> usize {
    ...
    map.values()
        ...
        .count() // Count the number of filtered values
}
...
fn count_collection_iterator(collection: &[HashMap<String, Progress>], value: Progress) -> usize {
    ...
    //               { "variables2": Complete, … }, … ]
    collection
        .iter() // Iterate over each hash map in the collection
        .flat_map(|map| map.values()) // Flatten the values of all hash maps into a single iterator
        .filter(|&&val| val == value) // Filter values that match the given `value`
        .count() // Count the number of filtered values
}
